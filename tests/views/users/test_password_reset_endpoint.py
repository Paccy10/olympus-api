...
from api.utils.helpers.messages.success import (PASSWORD_UPDATED,
...
class TestUserPasswordReset:
    ...
    def test_reset_request_succeeds(self, client, init_db, new_user):
        ...
        response = client.post(
            f'{API_BASE_URL}/users/reset-password', data=request_data, content_type=CONTENT_TYPE)

        assert response.status_code == 200
        assert response.json['status'] == 'success'
        assert response.json['message'] == PASSWORD_RESET_LINK_SENT
